			...
			success: function(data) {
				...
				data.list.forEach(function(value, index) {
					if (data.list[index].honorSid == newsSid) {
						var newsAuthor = data.list[index].honorAuthor || deAuthor;
						//console.log(data.list[index].honorIntroduce)
						html += '<div class="news_headl">' +
							'<h1>' + data.list[index].hornorTitle + '</h1>' +
							'<div class="news_summary">' +
							'<p>来源：' + newsAuthor + '<span>时间：' + data.list[index].honerTime + '</span><span>浏览：' + data.list[index].honorReaders + '</span></p>' +
							'</div>' +
							'</div>' +
							'<div class="news_text">' +
							'<p>' + data.list[index].honorIntroduce + '</p>' +
							'</div>';
						document.title = data.list[index].hornorTitle;
					}
				});
				$(".news_middlel").html(html);
				/*for (var i = 0; i < data.list.length; i++) {
